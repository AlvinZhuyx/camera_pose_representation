...
def construct_block_diagonal_weights(num_block, block_size, name='M', antisym=False):
    if not antisym:
        M = tf.Variable(tf.random_normal([num_block, block_size, block_size], stddev=0.001), name=name)
    else:
        M = tf.Variable(tf.random_normal([num_block, int(block_size * (block_size-1) / 2)], stddev=0.001), name=name)
