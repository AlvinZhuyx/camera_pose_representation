...
def calc_vos_simple(poses):
  ...
  vos = []
  for p in poses:
    pvos = [p[i+1].unsqueeze(0) - p[i].unsqueeze(0) for i in np.arange(len(p)-1)]
...
def calc_vos_safe_fc(poses):
  ...
  for p in poses:
    pvos = []
    for i in np.arange(p.size(0)):
      for j in np.arange(i+1, p.size(0)):
...
def process_poses(poses_in, mean_t, std_t, align_R, align_t, align_s):
  ...
  # align
  for i in np.arange(len(poses_out)):
...
class PoseGraph:
  ...
  def optimize(self, poses, vos, sax=1, saq=1, srx=1, srq=1, n_iters=10):
    ...
    for n_iter in np.arange(n_iters):
